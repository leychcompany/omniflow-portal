...
export function EditManualModal({ open, manualId, onOpenChange, onSuccess }: EditManualModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const getHeaders = async (): Promise<Record<string, string>> => {
        ...
        return s?.access_token ? { Authorization: `Bearer ${s.access_token}` } : ({} as Record<string, string>)
      }
      let res: Response
    ...
    <AdminAddModalLayout maxWidth="xl" onBackdropClick={handleClose}>
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="max-h-[min(85vh,900px)] w-full min-w-0 space-y-5 overflow-y-auto pt-6 pr-1">
            {loading && (
                ...
                <div>
                  ...
                  {form.download_url && (
                    <div className="mb-4 w-full min-w-0 overflow-hidden rounded-xl border border-zinc-200 dark:border-white/[0.1]">
                      ...
                      {/* Hash zoom=page-width so Chrome/Edge fit the page to iframe width */}
                      <div
                        className="relative w-full min-w-0 bg-[#525659]"
                        style={{
