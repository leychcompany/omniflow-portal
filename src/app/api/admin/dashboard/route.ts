...
export async function GET(request: NextRequest) {
  ...
  try {
    const [
      ...
      supabaseAdmin.from("activity_events").select("event_type, created_at").gte("created_at", since14d).order("created_at", { ascending: true }),
      Promise.all([
        supabaseAdmin.from("users").select("id").eq("role", "admin"),
        supabaseAdmin.from("activity_events").select("id, event_type, user_id, resource_type, resource_id, metadata, created_at").order("created_at", { ascending: false }).limit(15),
      ]),
    ...
    const recentActivityRaw = (adminAndRecentRes[1] as { data?: { id: string; user_id: string; event_type: string; resource_type: string | null; resource_id: string | null; metadata: unknown; created_at: string }[] }).data ?? [];
